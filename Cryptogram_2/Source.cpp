...
void del(int* original, int& length, const char& c, const int& x, const int& y);
void display ( int arr[], int length);

int main(){
	ios::sync_with_stdio(false);
	freopen("pwd2_test_input.txt", "r", stdin);
	for(int case_index = 0 ; case_index<TEST_CASE ; case_index++){
		...
		for (int i = 0; i < command_length; i++){
			...
			if(c == 'D'){
				del(original_cryptograms, cryptogram_length, c, x, y);
			}
